export function getTransformationUrl(options) {
    ...
    const cloud = cloudName || (typeof window !== 'undefined' && window.location.hostname.includes('localhost') ? process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME : '');
    if (!cloud) {
        console.warn('Cloud name not provided and NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME not set');
        return '';
    }
    let transformations = {};
